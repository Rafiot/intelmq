...
class MISPParserBot(Bot):
    ...
    def get_internal_reference(self, attributes):
        # Kind of dirty, but this is the only way to set a RTIR reference
        for attribute in attributes:
            if attribute['category'] == 'Internal reference' and attribute['value'].isdigit():
    ...
    def process(self):
        ...
        internal_ref = self.get_internal_reference(event_attributes)
        ...
        for attribute in event_attributes:
            ...
            if (category in self.SUPPORTED_MISP_CATEGORIES and
                ...
                if internal_ref:
                    event.add('rtir_id', internal_ref)
